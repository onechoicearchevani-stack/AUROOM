...
const orderLimiter = rateLimit({
  ...
  legacyHeaders: false,
  validate: false,
  // Key by table number, not IP: in a restaurant every guest shares the same
  // Wi-Fi (one public IP), so an IP-based limit would block the whole venue.
  // Per-table limiting still stops a single table from spamming.
  keyGenerator: (req) => {
    const t = req.body && req.body.tableNumber;
    return t != null && String(t).trim() !== '' ? 'table:' + String(t).trim() : (req.ip || 'global');
  },
...
router.post('/', orderLimiter, (req, res) => {
  ...
  const tableExists =
    ...
    db.prepare('SELECT 1 FROM restaurant_tables WHERE table_number = ?').get(table);
  if (!tableExists) {
